...
//                """;
    ...
    public void listRangeTransactions(LocalDate fromDate, LocalDate toDate) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            while (resultSet.next()) {
                ...
                Timestamp transactionDate = resultSet.getTimestamp("transaction_date");
                String type = resultSet.getString("type");
                System.out.println(cardNumber+" "+ address+" "+ amount+" "+ transactionDate+" "+ type);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void payment(TransactionDTO transactionDTO){
        ...
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("insert into transactions(cardid,amount,terminal_id,type) values (?,?,?,?)");
            preparedStatement.setInt(1,transactionDTO.getCardId());
            preparedStatement.setLong(2,transactionDTO.getAmount());
            preparedStatement.setInt(3,transactionDTO.getTerminalId());
            preparedStatement.setString(4,transactionDTO.getType().name());
